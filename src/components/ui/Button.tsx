...
export function Button({ variant = 'primary', size = 'md', loading, block, className, children, disabled, ...rest }: Props) {
  return (
    <button
      ...
      className={cx(
        'inline-flex items-center justify-center gap-2 rounded font-medium transition-colors duration-150 select-none whitespace-nowrap disabled:cursor-not-allowed',
        variants[variant],
        sizes[size],
        block && 'w-full',
        className,
